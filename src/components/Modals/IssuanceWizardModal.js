            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[94vh] flex flex-col overflow-hidden">
                ...
                <div className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white px-4 sm:px-6 py-3.5 sm:py-4 flex justify-between items-center shrink-0">
                    <div>
                        ...
                        <p className="text-emerald-200 text-xs sm:text-sm mt-0.5">{request.serial_number} — {request.beneficiary_name}</p>
                    </div>
                ...
                {/* Progress Steps */}
                <div className="px-3 sm:px-6 py-2.5 sm:py-3 border-b border-slate-200 bg-slate-50 flex items-center gap-2 sm:gap-3 shrink-0 overflow-x-auto">
                    {stepTitles.map((title, i) => (
                        <React.Fragment key={i}>
                            <div className={`flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm font-medium shrink-0 ${step > i + 1 ? 'text-emerald-600' : step === i + 1 ? 'text-slate-900' : 'text-slate-400'}`}>
                                <span className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold ${step > i + 1 ? 'bg-emerald-600 text-white' : step === i + 1 ? 'bg-slate-900 text-white' : 'bg-slate-200 text-slate-500'}`}>
                                    {step > i + 1 ? <Check className="w-3 h-3 sm:w-3.5 sm:h-3.5" /> : i + 1}
                                </span>
                                <span>{title}</span>
                            </div>
                ...
                {/* Footer */}
                <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-slate-200 bg-slate-50 flex flex-col-reverse sm:flex-row sm:justify-between items-stretch sm:items-center gap-2 sm:gap-0 shrink-0">
                    <button
                        disabled={issuanceCompleted}
                        onClick={() => (step === 1 || (step === 2 && isReserved)) ? handleClose() : setStep(step - 1)}
                        className="flex items-center justify-center gap-2 px-4 py-2 text-xs sm:text-sm font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
                    >
                        ...
                        <div className="flex flex-col sm:flex-row gap-2">
                            ...
                            {step === 1 && selectedFacility?.type === 'facility' && request.status === 'APPROVED_INTERNAL' && (
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 px-4 py-2 bg-amber-50 border-2 border-amber-300 text-amber-800 text-xs sm:text-sm font-bold rounded-xl hover:bg-amber-100 disabled:opacity-50 transition-all"
                                >
                            ...
                            <button
                                ...
                                className="flex items-center justify-center gap-2 px-6 py-2 bg-slate-900 text-white text-xs sm:text-sm font-bold rounded-xl hover:bg-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                            >
                        ...
                        (() => {
                            ...
                            return (
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 px-6 py-2.5 bg-emerald-600 text-white text-xs sm:text-sm font-bold rounded-xl hover:bg-emerald-700 disabled:opacity-30 disabled:cursor-not-allowed shadow-lg transition-all"
                                >
                                    {isExecuting ? (
