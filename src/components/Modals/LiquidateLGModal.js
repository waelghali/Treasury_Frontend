...
const LiquidateLGModal = ({ lgRecord, onClose, onSuccess, isGracePeriod }) => {
    const [liquidationType, setLiquidationType] = useState('full');
    const [supportingDocument, setSupportingDocument] = useState(null);
    const [isSubmitting, setIsSubmitting] = useState(false); // CRITICAL FIX: Add local state for submission status
    ...
    const handleSubmit = async (values, { setErrors }) => {
        ...
        try {
            ...
            const response = await apiRequest(`/end-user/lg-records/${lgRecord.id}/liquidate`, 'POST', formData);
                        ...
                        >
							...
							<div className="absolute right-3 top-3 sm:right-4 sm:top-4 z-10">
                                ...
                                <div className="w-full">
                                    <DialogTitle as="h3" className="text-lg font-bold leading-6 text-gray-900 tracking-tight border-b pb-3 mb-2 pr-8">
                                        ...
                                        >
                                            {({ errors, touched, values, setFieldValue }) => (
                                                <Form className={`space-y-4 ${isGracePeriod ? 'opacity-50' : ''}`}>
                                                    ...
                                                    <div>
                                                        <label htmlFor="liquidationType" className="block text-xs sm:text-sm font-medium text-gray-700">
                                                            Liquidation Type
                                                        </label>
                                                        <Field
                                                            as="select"
                                                            ...
                                                            className={`mt-1 block w-full px-3 py-2 text-sm rounded-lg border border-gray-300 ${errors.liquidationType && touched.liquidationType ? 'border-red-500' : 'border-gray-300'}`}
                                                    ...
                                                    {liquidationType === 'partial' && (
                                                        <div>
                                                            <label htmlFor="newAmount" className="block text-xs sm:text-sm font-medium text-gray-700">
                                                                New LG Amount <span className="text-red-500">*</span>
                                                            </label>
                                                            <Field
                                                                type="number"
                                                                id="newAmount"
                                                                ...
                                                                className={`mt-1 block w-full px-3 py-2 text-sm rounded-lg border border-gray-300 ${errors.newAmount && touched.newAmount ? 'border-red-500' : 'border-gray-300'}`}
                                                                disabled={isGracePeriod}
                                                            />
                                                    ...
                                                    <div>
                                                        <label htmlFor="reason" className="block text-xs sm:text-sm font-medium text-gray-700">
                                                            Reason for Liquidation <span className="text-red-500">*</span>
                                                        </label>
                                                        <Field
                                                            as="textarea"
                                                            id="reason"
                                                    ...
                                                    <div className="mt-6 flex flex-col-reverse sm:flex-row sm:justify-end gap-2.5 sm:gap-3 pt-2 border-t border-gray-100">
                                                        ...
                                                        <GracePeriodTooltip isGracePeriod={isGracePeriod}>
                                                            <button
                                                                type="submit"
                                                                className={`${buttonBaseClassNames} justify-center w-full sm:w-auto bg-red-600 text-white hover:bg-red-700 py-2.5 ${isSubmitting || isGracePeriod ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                                disabled={isSubmitting || isGracePeriod}
                                                            >
                                                                {isSubmitting ? <Loader2 className="h-5 w-5 mr-2 animate-spin" /> : <FileMinus className="h-5 w-5 mr-2" />}
