...
// Known Entity Types for quick filter select
const ENTITY_TYPES = [
  'ALL',
  'Customer',
  'User',
...
function AuditLogs() {
  ...
  useEffect(() => {
    apiRequest('/system-owner/customers', 'GET')
      .then((data) => setCustomers(data || []))
      .catch((err) => console.warn('Could not load customers for audit filters:', err));
  }, []);
  ...
  // Refetch when main dropdown filters or dates change (debouncing search is handled or on enter)
  useEffect(() => {
    fetchAuditLogs();
  }, [customerFilter, actionFilter, entityFilter, startDate, endDate]);
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const fullUrl = `${API_BASE_URL}/system-owner/audit-logs/export-csv?${queryParams.toString()}`;
      ...
      a.href = url;
      a.download = `system_audit_logs_${moment().format('YYYY-MM-DD')}.csv`;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Complete security, authentication, and entity modification activity across all customer tenants
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="bg-white p-4 rounded-xl border border-blue-200 shadow-sm">
          <p className="text-xs text-blue-600 font-bold uppercase">Updates & Configs</p>
      ...
      {isLoading ? (
        <div className="text-center py-16 bg-white rounded-xl border border-slate-200 shadow-sm">
          ...
          <p className="text-slate-500 mt-2 text-sm">Loading audit logs...</p>
        </div>
      ) : logs.length === 0 ? (
            ...
            <table className="min-w-full divide-y divide-slate-100">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3 text-left text-[10px] font-bold text-slate-500 uppercase tracking-wider">Customer</th>
                  ...
                  <tr key={log.id} className="hover:bg-blue-50/40 transition-colors">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="font-semibold text-slate-900 truncate max-w-[180px]" title={log.user_name}>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {log.customer_name ? (
                        ...
                        <span className="text-slate-500 font-mono text-[11px]">Cust #{log.customer_id}</span>
                      ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-2xl w-full p-6 space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs bg-slate-50 p-3 rounded-xl border border-slate-100">
              ...
              <div>
                ...
                <span className="font-semibold text-slate-800">{selectedLog.customer_name || (selectedLog.customer_id ? `Customer #${selectedLog.customer_id}` : 'Global')}</span>
              </div>
              <div>
