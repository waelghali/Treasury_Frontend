...
function SystemOwnerDashboard({ onLogout }) {
  ...
  const fetchAllDashboardData = async () => {
    ...
    setError('');
    try {
      const [dashboardMetrics, usageMetrics, feedbackRes] = await Promise.all([
  ...
  const metricsToDisplay = [
    ...
    },
    {
      ...
      changeColor: 'text-amber-600 bg-amber-50 dark:bg-amber-950/40'
    },
    {
      id: 4,
  ...
  const quickActions = [
    ...
    { id: 2, title: 'Plans & Tiers', description: 'Billing quotas', icon: Briefcase, link: '/system-owner/subscription-plans', color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-950/50' },
    { id: 3, title: 'Bank Forms', description: 'AI overlays', icon: FileCheck, link: '/system-owner/bank-forms', color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-950/50' },
    { id: 4, title: 'Global Configs', description: 'Timers & policies', icon: Settings, link: '/system-owner/global-configurations', color: 'text-slate-600', bg: 'bg-slate-50 dark:bg-slate-900' },
  ];
  ...
  return (
    <div className="flex flex-col h-full space-y-3 overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-300">
              Cross-tenant telemetry, subscription governance, and AI Assistant listener.
            </p>
          </div>
        </div>
      ...
      {/* Top 4 Executive KPI Cards (Compact Row) */}
      <div className="flex-shrink-0 grid grid-cols-2 sm:grid-cols-4 gap-2.5">
        {metricsToDisplay.map((metric) => (
          ...
          >
            ...
            <div className={`p-2.5 rounded-xl bg-gradient-to-br ${metric.gradient} text-white shadow-xs`}>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-3 min-h-0">
        ...
        <div className="lg:col-span-7 flex flex-col space-y-3 min-h-0">
          ...
          <div className="flex-1 bg-white dark:bg-slate-800 p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs flex flex-col justify-between min-h-0">
            ...
            {/* Quick Banner for Feedback */}
            <div className="mt-2 bg-gradient-to-r from-purple-900 to-indigo-950 p-2.5 rounded-xl text-white flex items-center justify-between border border-purple-800">
              <div className="flex items-center gap-2">
                <MessageSquare className="w-4 h-4 text-purple-300" />
                <span className="text-[11px] font-semibold text-purple-100">
                  User Suggestions & AI Problem Listener
                </span>
              </div>
              <Link
                to="/system-owner/feedback"
                className="text-[11px] font-bold text-purple-200 hover:text-white underline decoration-purple-400"
              >
                Open Inbox →
              </Link>
            </div>
